...
function showFlag(){
  ...
  myDecipher = decipher('mySecretSalt');
  FLAG = myDecipher("4b4a43734f386c5b6339646457387a574b603b3c7c6d7a75");
  alert(FLAG);
}
...
function MarioGame() {
  ...
  this.showInstructions = function() {
    ...
    gameUI.writeText('Tip: Jumping while running makes you jump higher but can you reach the hidden flag?(not at level 3)', 30, 60);
  };
      ...
      if (tickCounter > maxTick) {
        ...
        timeOutId = setTimeout(function() {
          ...
          if (originalMaps[currentLevel]) {
            ...
            score.updateLevelNum(currentLevel);
            if (currentLevel == 1337 && score.coinScore == 1337) {
              showFlag();
            }
          } else {
            that.gameOver();
